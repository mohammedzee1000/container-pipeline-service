...
This module contains implementation of Container dependency graph for the
container pipeline service, as well aS dependency validator.
"""
...
class ContainerDependencyGraph(object):
    """
    This class encapsulates a python-networkx Directed Graph which
    is used to maintain
    ...
    def __init__(self):
        ...
        # Networkx represents each node as a list whose 1st element is
        # a number to uniquely identify the node, and also help connect
        # the edges, and the 2nd element is a dict which holds attributes
        # which can be attached to each node.
        self._dependency_graph = nx.DiGraph()
        self._node_number = 1

    def get_container_node(self, container_id=None, container_name=None):
        """
        Gets the node, identified by container_name from the graph, if
        it exists.
        :param container_id: The id of the container, you want to get node for
        :param container_name: The name of the container, you want
        """
        node = None
        ...
        if not container_id and not container_name:
            return None, "Either the id or the name of the container must be " \
                         "specified"
        elif not container_id and container_name:
            for item in nodes:
                # Check the name label associated with the node to check if
                # its the container_name we are looking for
    ...
    def add_container(self, container_name, from_index=False,
                      from_target_file=False):
        """
        Checks if a node exists for container_name, and adds it if it does not exist
        :param container_name: Name of the container
        :param from_index: set to true, if the container was found in the container
        index
        :param from_target_file: set to true, if container was found in target-file
        """
        ...
        if not node:
            ...
            # Otherwise update the attributes of node to indicate from_index and
            # from_target_file
    ...
    def add_dependency(self, from_container, to_container):
        """
        Add a dependency between the containers as a directed edge from one container
        to the other.
        """
        ...
        err_names = []
        # Check if nodes are available for both the containers, if not then, they
        # should have been added already
        if from_node and to_node:
            # Extract he numbers which uniquely identify the node, so that an edge can
            # be formed between them, if not already present
            f = from_node[0]
            t = to_node[0]
            # Get the edges from the graph and check is an edge does not already exist
            # for the pair, if not add it now.
    ...
    def dependency_exists(self, from_container, to_container):
        ...
        # should have been added already
        if from_node and to_node:
            # Extract he numbers which uniquely identify the node, so that an edge
            # can be formed between them, if not already present
            f = from_node[0]
            t = to_node[0]
            # Get the edges from the graph and check is an edge exist for the pair.
            # If not, then no dependency
            edges = self._dependency_graph.edges()
            if (f, t) in edges:
                return True
        else:
    ...
    def _resolve_dependencies(dependencymap):
        """
        Dependency resolver
        :param dependencymap: is a dependency dictionary in which
        ...
        while d:
            ...
            # Basically if an item x is in values, then it has another item y in the
            # keys which depends on x : x -> y , but if x is not in keys, then it does
            # not have anyone that it depends on
            t = set(i for v in d.values() for i in v) - set(d.keys())
            # and keys without value (items without dep)
            # Basically if an item x has no items in the values, then it does not have
            # any item that it depends on
            t.update(k for k, v in d.items() if not v)
            # can be done right away
            r.append(t)
            # and cleaned up
            # Here, read every key value pair in the dict, and process further if the v
            # is not empty (k has dependencies) So basically we only take those items
            # forward that that dependencies, create a new dict such that for every key,
            # the value does not contain items that have been added to t, as they are a
            # part of that set.
    ...
    def get_processing_order(self):
        """
        Traverses the dependency graph and gets the order in which the nodes can be
        processed, as a list of lists.
        """
        # Create a dependency map by traversing the graph, see _resolve_dependencies for
        # more information
        ...
        for node in nodes:
            ...
            dependency_map[node] = []
            # Check if there are any nodes that have an edge from them to current node
            # respectively. This indicates dependency from any node to current node
            for edge in edges:
                if node == edge[1]:
